var generate = () => {// function to generate a random joke

    var random_number = Number.parseInt(Math.random() * (jokes.length - 1));//generates a random number which is lesser than the length of the array size
    var ele = document.body.getElementsByTagName("textarea")[0]; // selecting the html element by tagname
    ele.innerHTML = jokes[random_number];// changing the innerHTML content to the array index
    ele.style.fontWeight = '900'; //styling
    ele.style.color = '#000';
  
    var button = document.body.querySelector('#btn') 
    button.firstElementChild.innerHTML = "Generate One More"; // renaming the button title
}
// array of jokes
var jokes = [`What did one hat say to the other?
